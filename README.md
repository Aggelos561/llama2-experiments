...
## LLAMA2 Optimized: Profiling, Multithreading and Performance Optimizations

I began by **profiling** the original code (run.c) to locate performance bottlenecks using perf record. 

Then, I experimented with a series of optimizations using OpenMP, AVX instructions, OpenBLAS integration, GPUs on CUDA and ROCm. Finally, I conducted AVX256 & AVX512 experiments on an EC2 instance on AWS, on an Intel Xeon Platinum 8488C CPU.

### Optimizations

The following experimentations we conducted

| Method              | Description                                                                                           |
|---------------------|-------------------------------------------------------------------------------------------------------|
| `-O3 -march=native` | Basic compiler optimizations and instruction set tuning, single-threaded                              |
| **OpenMP**          | Parallelized matrix operations across CPU cores using `#pragma omp` directives                        |
| **CBLAS (OpenBLAS)**| Used optimized BLAS matrix multiplication routines instead of the C implementation                    |
| **AVX**             | Enabled compiler vectorization using the `-mavx` flag (using default matrix multiplication)           |
| **AVX256**          | Manually implemented 256-bit SIMD matrix multiplication using AVX intrinsics (`__m256`)               |
| **AVX512**          | Manually implemented 512-bit SIMD matrix multiplication using AVX-512 intrinsics (`__m512`)           |
| **CUDA**            | GPU acceleration using NVIDIA's CUDA and cuBLAS (from from [rogerallen/llama2.cu](https://github.com/rogerallen/llama2.cu))|
| **ROCm**            | GPU acceleration using AMD's HIP and rocBLAS (ported from the CUDA implementation)                    |


### Dependencies

You will need the following package and/or hardware dependencies in order to run specific targets

- **AVX instructions compatible CPU**
  Required for targets using `avx`, `avx256`, `avx512`, `avxopenmp256`, and `avxopenmp512`.  

- **[OpenBLAS](http://www.openmathlib.org/OpenBLAS/)**  
  Required for builds that use `CBLAS` or `cblasopenmp`.  

- **[NVIDIA CUDA Toolkit](https://developer.nvidia.com/cuda-toolkit)**  
  Required for the `run_cuda` target.  

- **[AMD ROCm](https://rocm.docs.amd.com/en/latest/)**  
  Required for the `run_rocm` target.  


### `Makefile` Targets

The projects `Makefile` compilation target variants are the following:

| Target              | Description                                                                     |
|---------------------|---------------------------------------------------------------------------------|
| `run`               | Baseline build with `-O3 -march=native` (no parallelism or SIMD)                |
| `openmp`            | Adds multithreading via OpenMP                                                  |
| `bmatopenmp`        | OpenMP with Blocked Matrix Multiplication algorithm                             |
| `cblas`             | Uses OpenBLAS for matrix multiplication (no multithreading)                     |
| `cblasopenmp`       | Combines OpenBLAS matmul with OpenMP for remaining concurrent operations        |
| `avx`               | Enables AVX via compiler vectorization (`-mavx`)                                |
| `avx256`            | Uses manual AVX256 intrinsics for 256-bit SIMD matrix multiplication            |
| `avx512`            | Uses manual AVX-512 intrinsics for 512-bit SIMD matrix multiplication           |
| `avxopenmp256`      | Combines AVX256 intrinsics with OpenMP for multithreaded SIMD                   |
| `avxopenmp512`      | Combines AVX-512 intrinsics with OpenMP for multithreaded SIMD                  |
| `run_rocm`          | Builds the ROCm variant using HIP and rocBLAS for AMD GPUs                      |
| `run_cuda`          | Builds the CUDA variant using cuBLAS for NVIDIA GPUs                            |


### Main Source Files

- **`run.c`**  
  The main C file used for CPU builds.  
  It includes the following implementations:
  - Default serial algorithm
  - OpenMP multithreading (all the configurations with it)
  - AVX256
  - AVX512
  - OpenBLAS (CBLAS) integration

- **`run_cuda.cu`**  
  CUDA implementation for NVIDIA GPUs.
  Based on a fork of [rogerallen/llama2.cu](https://github.com/rogerallen/llama2.cu).

- **`run_rocm.cpp`**  
  HIP/ROCm implementation targeting AMD GPUs.  
  Ported from the CUDA version and accelerated using rocBLAS.

Each file is used depending on the selected build target, as defined in the `Makefile`.


## LLAMA2 Models
Performance evaluation was conducted using the largest **[stories110M.bin](https://huggingface.co/karpathy/tinyllamas/resolve/main/stories110M.bin)** model. 
You can download all the available models using the `scripts/download_models.sh` bash script.


### Profiling

The profiling was done using the `perf record` tool in order to find the performance bottenecks in the code that consumed the most CPU cycles. 
This helped us identify the inference operations that required optimizations. The following Figure shows the functions that consume the most cpu cycles during the inference of the [stories110M.bin](https://huggingface.co/karpathy/tinyllamas/resolve/main/stories110M.bin) model. We observed that the majority of the overhead can be found in the forward function, and more specifically in the matrix multiplication routine, which is called repeatedly throughout the algorithm's execution. For this reason we mainly focus on optimizing the matrix multiplication function using tecnhiques that are listed above.

![Performance Plot](./scripts/Plots/perf-record.png)


### Optimization Tecnniques

#### Baseline
Baseline configuration is running the inference algorithm without any multi-threading technique. An optimization that was done here and also applied to the following as well is to set the `-march=native` gcc flag, to allow the compiler to generate more optimized ISA based on the machine's architecture.

#### OpenMP
This project already had OpenMP on matrix multiplication and on multi-head attention. We additionally applied OpenMP on `rmsnorm`, `softmax` functions, though they do not have significant overhead to improve the performance of the inference. However, OpenMP was also applied on the following optimizations.

#### OpenBLAS
OpenBLAS was used to replace the C matrix multiplication implementation with the `cblas_sgemv` BLAS routine. We also experimented with combining OpenBLAS and OpenMP (`make cblasopenmp`). However, since both OpenBLAS and OpenMP can create multiple threads, it's important to balance the number of threads each system uses to avoid thread contention.

#### AVX
To allow the GCC compiler to vectorize, the `-mavx` compiler flag was enabled. This flag may give some performance improvements without needing to change the code. In our case, although this flag generated SIMD instructions (in assembly), it did not provide any performance improvement on the baseline program.

Next, we manually implemented the matrix multiplication algorithm using the AVX intrinsics. In the `AVX256` version, we used 256-bit registers (`__m256`) to vectorize the algorithm. This allowed us to concurrently operate on 8 floating point numbers on every iteration and the sum it using `_mm256_mul_ps` and `_mm256_add_ps` functions. This optipmization gave us great performance improvements.

Similar to the `AVX256` methodology, the `AVX512` utilized the same technique using 512-bit wide registers (`__m512`), that allows operating on 16 float numbers in each iteration. However, our local machine did not support AVX512 instructions to experiment with, so we ran these experiments on AWS on an Intel Xeon Platinum 8488C.

For both `AVX256` and `AVX512` configurations, we also combined them with OpenMP (`avxopenmp256`, `avxopenmp512`) to achieve multi-threaded vectorized execution on the matrix multiplication function. 

#### Blocked Matrix Multiplication
Blocked matrix multiplication divides matrices into smaller blockes to improve cache locality. We applied OpenMP on the outer loop over matrix rows, for multiple threads to work on different rows concurrently. The blocking was done inside each thread with chunks of 16 floats (4 bytes each), making each block 64 bytes in to maximize the cache locality. More details on blocked mat mul can be found here: [Cache Blocking for Matrix Multiplication](https://csapp.cs.cmu.edu/2e/waside/waside-blocking.pdf).

#### CUDA
CUDA acceleration was introduced using the following project: [llama2.cu by rogerallen](https://github.com/rogerallen/llama2.cu). This project uses NVIDIA's CUDA language and the `cuBLAS` library for matrix multiplication. 

#### ROCm
Based on the CUDA implementation, we ported the code to AMD's ROCm using the HIP compiler. This port uses the `rocBLAS` (similar to Nvidia's `cuBLAS`) library to do the matrix multiplications on ROCm. 

As we will see on the performance evaluation graphs, GPUs provide a substantial performance improvement compared to the CPU optimization tecnhiques.


### Performance Evaluation

The evaluation was done using the [stories110M.bin](https://huggingface.co/karpathy/tinyllamas/resolve/main/stories110M.bin) model with temperature set to 0 (-t 0), which makes next token prediction deterministic by always selecting the most likely token and the randomization seed was set to 42 (-s 42). 

We evaluated the model on three hardware setups:

- **CPU**: AMD Ryzen 9 5900X, 64GB RAM (Native) | Intel Core Haswell Processor, 50GB RAM (VM) | Intel Xeon Platinum 8488C (EC2 AWS)
- **NVIDIA GPU**: Tesla V100 with CUDA (paired with Intel Core Haswell Processor)
- **AMD GPU**: Radeon RX 7900 XTX (gfx1100) on ROCm (paired with AMD Ryzen 9 5900X)

For each target configuration, we:

- Ran **each configuration 10 times** and reported **average metrics**
- Evaluated with **thread counts from 1 to 16**, incrementing by 2 on CPU configurations
- Checked the **balance of OpenMP threads and OpenBLAS threads** to avoid thread contention (`cblasopenmp`)

Figure 1 shows the performance results from 1 to 16 threads for all the CPU-based configurations. We notice that not all target configurations have their best performance at the same number of threads:

- **AVX + OpenMP**: Best performance with **6 threads**
- **Block Matrix Multiplication + OpenMP**: Best performance with **10 threads**
- **OpenBLAS (CBLAS)**: Best performance with **4 threads**
- **OpenMP**: Best performance with **10 threads**

We also observe that AVX256 combined with OpenMP multithreading has the best performance overall.

Figure 2 shows the experiments on balancing the number of threads for OpenMP and OpenBlas, since both require spawning multiple threads, and thread contention must be avoided. We notice that the best performance is with **4 threads** for each library totalling 8 threads.

![threads_evaluation](./scripts/Plots/threads-evaluation.png)
**Figure 1: Experimenting with threads number for each target configuration on AMD Ryzen 9 5900X**

![scaling_plot](./scripts/Plots/scaling_plot.png)
**Figure 2: Experimenting with the balance between OpenMP and OpenBLAS threads on AMD Ryzen 9 5900X**

Next, using the **best-performing number of threads (based on lowest average elapsed time)** for each target configuration, we show the following metrics:

- **Average elapsed time**
- **Average tokens per second**
- **Average time to first token (TTFT)**

Figures 3, 4, and 5 show the average elapsed time, tokens per second, and time to first token (TTFT), respectively. All CPU configurations and the ROCm experiments ran on the Ryzen 9 5900X, while the CUDA experiments were conducted on the system with the Intel Core Haswell processor. We notice that the GPU implementations achieve the best performance, with the lowest elapsed time, highest tokens per second, and relatively low TTFT latency compared to the other configurations. In addition, CUDA has better performance compared to the AMD ROCm implementation.

![avg_elapsed_plot](./scripts/Plots/avg_elapsed.png)  
**Figure 3: Average elapsed time for different configuration targets and system setups**

![avg_tokps_plot](./scripts/Plots/avg_tokps.png)  
**Figure 4: Average tokens generated per second for each configuration and system setup**

![avg_ttft_plot](./scripts/Plots/avg_ttft.png)  
**Figure 5: Average time to first token (TTFT) for each configuration and system setup**

Finally, we ran AVX256 and AVX512 with OpenMP on an **AWS EC2 machine**. Figure 6 shows the performance results scaling from 1 to 16 threads. We observe that the two configurations have very similar performance, with **AVX512 performing slightly better** across most threads runs.

![xeon_avg_elapsed_plot](./scripts/Plots/xeon-threads-evaluation.png)  
**Figure 6: Average elapsed time for AVX256 & AVX512 with OpenMP on AWS**
...
```
make run
./run ./models/stories110M.bin -t 0 -s 42
```
...
```
make run_cuda
./run_cuda ./models/stories110M.bin -t 0 -s 42
```

#### 9. AMD ROCm
```
make run_rocm
./run_rocm ./models/stories110M.bin -t 0 -s 42
```

**Directory `./scripts` contains all the scripts to benchmark and plot the performance of all the configurations described above.**
