/* Inference for Llama-2 Transformer model in ROCm C */
...
void malloc_run_state(RunState* s, Config* p) {
    ...
    HIP_CHECK(hipMalloc((void**)&s->x, p->dim * sizeof(float)));

    HIP_CHECK(hipMalloc((void**)&s->xb, p->dim * sizeof(float)));

    HIP_CHECK(hipMalloc((void**)&s->xb2, p->dim * sizeof(float)));

    HIP_CHECK(hipMalloc((void**)&s->hb, p->hidden_dim * sizeof(float)));

    HIP_CHECK(hipMalloc((void**)&s->hb2, p->hidden_dim * sizeof(float)));

    HIP_CHECK(hipMalloc((void**)&s->q, p->dim * sizeof(float)));

    HIP_CHECK(hipMalloc((void**)&s->key_cache, p->n_layers * p->seq_len * kv_dim * sizeof(float)));

    HIP_CHECK(hipMalloc((void**)&s->value_cache, p->n_layers * p->seq_len * kv_dim * sizeof(float)));

    HIP_CHECK(hipMalloc((void**)&s->att, p->n_heads * p->seq_len * sizeof(float)));

    HIP_CHECK(hipMalloc((void**)&s->logits, p->vocab_size * sizeof(float)));

    HIP_CHECK(hipMalloc((void**)&s->logits_gpu, p->vocab_size * sizeof(float)));

}

void free_run_state(RunState* s) {
    HIP_CHECK(hipFree(s->x));
    HIP_CHECK(hipFree(s->xb));
    HIP_CHECK(hipFree(s->xb2));
    HIP_CHECK(hipFree(s->hb));
    HIP_CHECK(hipFree(s->hb2));
...
void read_checkpoint(char* checkpoint, Config* config, TransformerWeights* weights,
    ...
    *data = (float*) mmap(NULL, *file_size, PROT_READ, MAP_PRIVATE, *fd, 0);
    ...
    HIP_CHECK(hipMalloc((void**)&weights_ptr, weights_size));
    HIP_CHECK(hipMemcpy(weights_ptr, *data + sizeof(Config)/sizeof(float), weights_size, hipMemcpyHostToDevice));
...
void free_transformer(Transformer* t) {
    ...
    HIP_CHECK(hipFree(t->weights.token_embedding_table));
...
__global__ void rmsnorm_kernel(float* o, float* x, float* weight, int size, int elementsPerThread) {
    ...
    using BlockReduce = hipcub::BlockReduce<float, num_threads_lrg>;
...
void rmsnorm(float* o, float* x, float* weight, int size) {
    int elements_per_thread = divUp(size, num_threads_lrg);
    rmsnorm_kernel <<<1, num_threads_lrg >>> (o, x, weight, size, elements_per_thread);
}
...
void matmul(float* xout, float* x, float* w, int n, int d) {
    ...
    float alpha = 1.0f;
    float beta = 0.0f;
    rocblas_sgemv(handle, rocblas_operation_transpose, n, d, &alpha, w, n, x, 1, &beta, xout, 1);
}
...
float* forward(Transformer* transformer, int token, int pos) {
    ...
    HIP_CHECK(hipMemcpy(x, content_row, dim*sizeof(*x), hipMemcpyHostToDevice));
    ...
    HIP_CHECK(hipMemcpy(s->logits, s->logits_gpu, p->vocab_size * sizeof(float), hipMemcpyDeviceToHost));
...
int argmax(float* v, int n) {
    ...
    HIP_CHECK(hipMalloc((void**)&pmax_pos, sizeof(int)));

    // call the kernel
    argmax_kernel<<<1, num_threads_lrg>>>(v, n, pmax_pos);

    // copy the result back to host
    HIP_CHECK(hipMemcpy(&max_pos, pmax_pos, sizeof(int), hipMemcpyDeviceToHost));

    // free the allocated memory
    HIP_CHECK(hipFree(pmax_pos));
...
void generate(Transformer *transformer, Tokenizer *tokenizer, Sampler *sampler, char *prompt, int steps) {
    __int8_t first_token_printed = 0;
    long total_time_ttft = 0;
    long ttft_start = time_in_ms();
    ...
    while (pos < steps) {
        ...
        if (!first_token_printed) {
            ...
            total_time_ttft = time_in_ms() - ttft_start;
        }
